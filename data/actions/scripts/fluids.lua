...
function onUse(cid, item, frompos, item2, topos)
	...
	if item2.itemid == 1 and item2.uid == cid then
		...
		elseif item.type == 26 then
			doSendAnimatedText(getPlayerPosition(cid), "Ouch!", 198)
			doTargetCombatCondition(0, cid, fire, CONST_ME_NONE)
		else
			doSendAnimatedText(getPlayerPosition(cid), "Gulp.", 198)
		end
	...
	elseif item2.uid == 0 then
		...
		if item.type == 0 then
			...
			if topos.x == 65535 then
				...
				if doDecayItem(splash) == -1 then
					debugPrint("1:x:"..topos.x.." y:"..topos.y.." z:"..topos.z.." item:"..splash.." "..getPlayerName(cid))
				end
			elseif isInArray(WATER, item2.itemid) == TRUE or isInArray(MUD, item2.itemid) == TRUE or isInArray(LAVA, item2.itemid) == TRUE or isInArray(SWAMP, item2.itemid) == TRUE or item2.itemid == 1777 then
				return 0
			else
				doChangeTypeItem(item.uid, 0)
				splash = doCreateItem(2025, item.type, topos)
				if doDecayItem(splash) == -1 then
					debugPrint("2:x:"..topos.x.." y:"..topos.y.." z:"..topos.z.." item:"..splash.." "..getPlayerName(cid))
				end 
			end
		end
	end
